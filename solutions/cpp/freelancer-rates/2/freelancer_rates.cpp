...
double apply_discount(double before_discount, double discount) {
    ...
    double afterDiscount = before_discount * (1 - discount / 100);    
...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    double monthResult = apply_discount(daily_rate(hourly_rate), discount) * 22;
    return ceil(monthResult);
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    int result = budget / apply_discount(daily_rate(hourly_rate), discount);
    return result;
}
